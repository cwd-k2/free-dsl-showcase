...
import type { Handler, Interpreter } from "../free.ts";
...
/** An empty query state, also used by interpreters that embed the SQL language. */
export function emptyQueryState(): QueryState {
  return { joins: [], where: [], select: [], orderBy: [] };
}

/** SQL handlers are exported internally so a larger domain interpreter can embed this language. */
export function sqlHandlers(): Record<string, Handler<QueryState>> {
  return {
    ...
    "sql.logical": (state, { op, parts }) => ({
      state,
      value: { tag: "logical", op, parts } satisfies SqlExpr,
    }),
